...
RelationType StrType::ComparedTo(StrType& otherString) {
    if (strcmp(letters, otherString.letters) == 0) return EQUAL;
    else if (strcmp(letters, otherString.letters) < 0) return LESS;
    else return GREATER;
}
*/
// 5번 문제 b번
RelationType StrType::ComparedTo(StrType& otherString) {
    int idx = 0;
    while (((strlen(letters) > idx) && (strlen(otherString.letters) > idx)) && (int(letters) == int(otherString.letters))) {
        idx++;
        if ((idx == strlen(letters)) && (idx == strlen(otherString.letters))) return EQUAL;
        if ((idx == strlen(letters)) && (idx < strlen(otherString.letters))) return LESS;
        if ((idx < strlen(letters)) && (idx == strlen(otherString.letters))) return GREATER;
    }
    if ((strlen(letters) == 0) || strlen(otherString.letters) == 0) {
        if ((strlen(letters) == 0) && (strlen(otherString.letters) > 0)) return LESS;
        if ((strlen(letters) > 0) && (strlen(otherString.letters) == 0)) return GREATER;
        if ((strlen(letters) == 0) && (strlen(otherString.letters) == 0)) return EQUAL;
    }
    if (int(letters[0]) != int(otherString.letters[0])) {
        if (int(letters[0]) > int(otherString.letters[0])) return GREATER;
        if (int(letters[0]) < int(otherString.letters[0])) return LESS;
    }
