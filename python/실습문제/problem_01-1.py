...
class MyQueue:
    ...
    def push(self, var):
        if var in self.original:
            errormsg = "error@duplicated"
            return errormsg
        if type(var) == int or type(var) == float:
            self.var = int(var)
            self.original.append(var)
            self.filtered.append(self.var)
            self.vardic[var] = self.var
        elif type(var) == str:
            ans = ''
    ...
    def pop(self, param = None):
        keylist = list(self.vardic.keys())
        valuelist = list(self.vardic.values())
        ans = {}
        if param is None:
            ans[keylist[-1]] = valuelist[-1]
            del self.vardic[keylist[-1]]
            del self.filtered[-1]
        else:
            index1 = keylist.index(str(param))
            a = valuelist[index1]
            ans[str(param)] = a
            del self.vardic[str(param)]
            del self.filtered[index1]
        return ans

    def get_dictionary(self):
        a = dict(zip(self.vardic.values(), self.vardic.keys()))
        return a

    def __add__(self, other):
        if type(self) == MyQueue and type(other) == MyQueue:
            my = MyQueue()
            temp = self.filtered + other.filtered
